...
- **Bugs and feature requests:** open an issue at
  ...
  reproducible example (input data frames plus the plotting call) makes a
  fix far more likely.
- **Parser issues:** if `read_mcscanx()`, `read_genespace()`, or
  `read_synteny_tsv()` mishandles a file, please attach (or excerpt) the
  offending file — real tool output is exactly what the parsers are
  tested against.

## Ground rules for pull requests

- **Light dependencies.** Hard Imports stay as they are; optional
  niceties go in `Suggests` behind `requireNamespace()` guards (see how
  ggiraph and ggforce are handled).
...
- Palette changes belong upstream in
  [ltc](https://github.com/loukesio/ltc-color-palettes); ggsynteny vendors
  the curated versions used by ggvmap so the ecosystem stays in sync.
